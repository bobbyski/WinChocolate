// Conditional C4 of the four sanctioned platform switches (see
// Docs/UnifiedChocolatePlan.md): the main-thread executor. Windows drives
// main-actor work from the Win32 message pump (Win32RunLoopPump); Linux drives
// it from GTK's loop, below. The `canImport(CGTK)` guard is what keeps this file
// inert everywhere GTK is not the backend.
#if canImport(CGTK)
import Foundation
import CGTK
#if canImport(Glibc)
