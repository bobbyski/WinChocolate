// A minimal XML document parser for Interface Builder `.xib` files — the
// LinChocolate port of WinChocolate's `WinXML` (Phase 15).
//
...
// all nib loading needs. Not a general XML library: namespaces, CDATA, and DTD
// internals are out of scope (IB emits none).

/// One parsed XML element: name, attributes, and child elements.
final class NibXMLElement {
    let name: String
    let attributes: [String: String]
    private(set) var children: [NibXMLElement] = []
    ...
    func addChild(_ child: NibXMLElement) {
...
enum NibXML {
    static func parse(_ text: String) -> NibXMLElement? {
    ...
    private struct Scanner {
        ...
        mutating func parseDocument() -> NibXMLElement? {
            skipProlog()
            return parseElement()
        }
        ...
        private mutating func parseElement() -> NibXMLElement? {
            skipWhitespace()
            ...
            while true {
                ...
                if consume("/") {
                    ...
                    guard consume(">") else { return nil }
                    return NibXMLElement(name: name, attributes: attributes)
                }
                if consume(">") {
                    break
                }
            ...
            // Children until the matching close tag. Text content (IB emits it
            // only for a few elements like <string>) is skipped — the nib
            // loader consumes attributes and structure.
            let element = NibXMLElement(name: name, attributes: attributes)
            while !isAtEnd {
                skipUntil("<")
                if matches("<!--") {
                    skipPast("-->")
        ...
        private mutating func readQuotedValue() -> String? {
            ...
            return NibXML.decodeEntities(String(raw))
        }
