/// A CoreGraphics-shaped bitmap image — the LinChocolate port of
/// WinCoreGraphics' `CGImage` (plan 13.6), kept API-identical so the shared
/// demo compiles against either library.
///
/// `CGImage` owns decoded pixels — width × height rows of RGBA8 — the
/// framework's honest *in-memory* bitmap representation. The first codec is
/// BMP (uncompressed 24/32-bit), matching WinCoreGraphics.
