...
| `Locale` | Partial | Backs the `DateFormatter`/`NumberFormatter` locale-aware presets. `Locale.current` reads the user's Windows locale name (`GetUserDefaultLocaleName`); exposes `identifier`, `shortDatePattern`/`timePattern`, and `decimalSeparator`/`groupingSeparator`/`currencySymbol` (all from `GetLocaleInfoEx`). Tests cover current-locale patterns, identifier storage, and locale-driven date/number formatting. | Add region/language components and full identifier↔Windows-name mapping as parity grows. |
| `NSNumber` | Partial | A reference-type numeric box (`final class`) that `NumberFormatter` and formatted controls take and return. Covers `init(value:)` for the common integer/floating/bool widths and `intValue`/`int32Value`/`int64Value`/`doubleValue`/`floatValue`/`boolValue`/`stringValue`, plus `compare(_:)`, value-based `==`/`hash`. Tests cover boxing, cross-width reads, equality, and `Set` hashing. | Automatic `Int as NSNumber` bridging is a Foundation feature the shim cannot reproduce; add `NSDecimalNumber`/`Decimal` and remaining accessors if needed. |
| `Formatter` | Partial | Abstract base for value formatters. Controls hold a `Formatter?` and call `string(for:)`/`editingString(for:)`. | Foundation's pointer-based `getObjectValue(_:for:errorDescription:)` (`AutoreleasingUnsafeMutablePointer`) can't be reproduced; parsing goes through the concrete subclass (`NumberFormatter.number(from:)`). |
| `NumberFormatter` | Partial | Formats/parses numbers for formatted fields and `objectValue` display. `numberStyle` presets (none/decimal/currency/percent) apply grouping + fraction-digit defaults; separators and the currency symbol come from `locale` (US on a US machine). `string(from:)`/`number(from:)`/`string(for:)`, configurable `minimum`/`maximumFractionDigits`, `minimumIntegerDigits`, `usesGroupingSeparator`, and explicit separator/symbol overrides. Contract-tested for each style and round-trip parsing, and verified live in the demo's currency field. | Scientific/spell-out styles, negative-format patterns, rounding modes, and `NSDecimalNumber` support are future work. |
...
| `Notification`, `NotificationCenter`, `OperationQueue` | Partial | Foundation-style loose event delivery for future AppKit notifications and app code that uses block observers. Current tests cover `Notification.Name`, synchronous block delivery, object filtering, wildcard observers, `userInfo`, `post(_:)`, `post(name:object:userInfo:)`, `OperationQueue.main`, observer removal, and asynchronous `OperationQueue.addOperation(_:)` execution used by WinCombine scheduling. | Add selector-based observer APIs only if WinChocolate grows an Objective-C-style selector bridge. NotificationCenter queue selection remains synchronous. |
