...
**Remaining polish:**
- Custom-drawn **text** (`String.draw`) and **images** (`NSImage.draw`) are still no-op stubs from the compile-to-zero pass — the Drawing page's "WinChocolate" label and phoenix image, and the Tables page's image view, don't appear yet (Pango text + Cairo image compositing are the follow-up).
