...
| Pop-up, segmented, date picker, table inert | `registerAction` was dispatched by widget kind so it would stop connecting `clicked` to things that have no such signal — but everything that was not a button, check box, text field or slider fell through to `default:` and got **no action at all**. AppKit puts target/action on `NSControl` and the core has no separate change registrar (there is no `registerSelectionAction` in the protocol), so target/action is the *only* channel these controls have. Each kind now routes to the change signal that is genuinely its action: `setSelectionChangeAction` for pop-up/segmented/tab/table, `setDateChangeAction`, `setValueChangeAction`, `setColorChangeAction`, `setTokensChangeAction`, `setScrollerAction`, `setClickAction` for plain views |
| Segmented control dead, hit-testing wrong everywhere | Mouse events carried GTK's **view-local** point as `locationInWindow`. The core hit-tests with it — `NSSegmentedControl.mouseDown` computes `event.locationInWindow.x - frameInWindow().origin.x` — so the difference went negative, no segment matched, and the control did nothing. Now converted through `gtk_widget_compute_point` to the window's space, then flipped to AppKit's bottom-left origin |
| Date picker blank | The compact picker is a text entry, and `setDateValue` deliberately does not write into it — formatting needs the locale, calendar and element flags, which are AppKit's to know. The core knows them and sends a pattern through `setDatePickerFormat`, which had nothing behind it. It now renders the current date through that pattern into the entry |

| ~1s click latency, everything sluggish | `scheduleNativeTimer` routed through the GTK layer's `scheduleTimer`, which returns no GLib source id — so the token handed back to the core mapped to nothing and `cancelNativeTimer` was a silent no-op. The core's contract is repeat-until-cancelled (`Timer.fire()` invalidates one-shots itself), so cancellation is the whole mechanism. `NSLayoutPump` arms a **1ms one-shot** `Timer` on every layout mark; each mark therefore minted an immortal 1ms repeating GLib timer. Measured with `LINCHOCOLATE_PAINT_TRACE`: ~1,900 timer fires/s sitting idle, +~1,000/s after each click. Now scheduled directly with `g_timeout_add_full`, source id stored per token, cancel really removes. After: ~27 trace lines/s idle (the demo's own two spinners + its 1s clock), 2.4% CPU, and a measured 141ms click→painted-label round trip — ~100ms of which is the measuring screenshot itself |

The last three are the shape to watch for in the rest of Phase 3: **a GTK method with
