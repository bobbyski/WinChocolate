...
| `NSScrollView` | `GtkScrolledWindow` | ✅ | `documentView` scrolls when larger than the frame; native overlay scrollbars. `hasVertical/HorizontalScroller` → `gtk_scrolled_window_set_policy` (automatic vs. never); `scroll(to:)` sets the `GtkAdjustment` value; `documentVisibleRect`; `onScroll` fires from the adjustments' `value-changed`. |
