...
| 3: AppKit Surface Expansion | Partial | 82% | menus, dialogs, responders, layout, text, tables, images | Initial `NSMenu`, `NSMenuItem`, `NSAlert`, `NSPanel`, `NSPopover`, `NSToolbar`, `NSToolbarItem`, `NSViewController`, `NSBox`, `NSColor`, `NSFont`, `NSEvent`, `NSResponder`, `NSApp`, `NSWindow.firstResponder`, key-view loop APIs, key/main window tracking, editable `NSTextField`, `NSSecureTextField`, `NSSearchField`, `NSTokenField`, `NSForm`, `NSMatrix`, `NSPathControl`, multiline `NSTextView`, `NSPopUpButton`, `NSComboBox`, `NSBrowser`, `NSCollectionView`, `NSImage`, `NSImageView`, `NSTabView`, `NSSlider`, `NSScroller`, `NSSegmentedControl`, `NSProgressIndicator`, `NSLevelIndicator`, `NSStepper`, `NSDatePicker`, `NSColorWell`, `NSClipView`, `NSScrollView`, `NSSplitView`, `NSVisualEffectView`, `NSTableColumn`, `NSTableView`, `NSOutlineView`, `NSCell`, `NSTableCellView`, `NSTableRowView`, `NSSortDescriptor`, and push/switch/radio `NSButton` APIs are present. `NSWindow.toolbar` now docks a classic `ToolbarWindow32` strip and reserves content layout space. |
| Foundation Bridge | Partial | 44% | `WinFoundation`, compile-time switch, URL/Data/Date/IndexSet/IndexPath/UUID/Bundle/NotificationCenter parity | First repo-local `WinFoundation` slice exists for `URL`, `Data`, `Date`, `IndexSet`, `IndexPath`, `UUID`, `Bundle`, `TimeInterval`, and synchronous `NotificationCenter`; Windows builds use it through `USE_WIN_FOUNDATION`, while `USE_REAL_FOUNDATION` forces real Foundation for toolchain verification. `FOUNDATION_SHIMS.md` tracks the shim contract and release canary. `URL` tests cover components, percent encoding, relative URLs with bases, path standardization, Windows drive roots, and UNC shares. `Data` covers byte collection/mutation APIs plus file URL read/write. `Date` uses a real Windows clock bridge. `IndexSet` covers range mutation and common set operations for table/list selection work. `IndexPath` covers collection section/item addressing. `UUID` uses native Windows GUID generation. `Bundle` covers main bundle and resource lookup basics. `NotificationCenter` covers block observer delivery and removal. |
| 4: Demo Application | Partial | 98% | SwiftPM demo app | Demo source builds as a SwiftPM executable and visibly exercises native state APIs, modal alerts, editable/secure/combo/token text, path display, multiline notes, checkbox state, radio groups, pop-up selection, segmented selection, bitmap image tests, clip-view scrolling, split view pane layout, tab selection, slider/progress/scroller/stepper/date values, table/outline selection/action, mouse events, and key events. |
| 5: AppKit Tables And Collection Controls | Partial | 39% | `NSTableView`, `NSOutlineView`, `NSBrowser`, collection/list selection, cells/views | First AppKit-shaped `NSTableView` slice exists with columns, data source, delegate, row and column selection helpers, sort descriptors, row/cell-view placeholders, scroll-view hosting, table action/double-action surface, tests, and a temporary classic backend renderer. First `NSOutlineView` flattening slice exists over the table backend. First composed `NSBrowser` column-selection slice exists. First `NSCollectionView` composed item-grid slice exists with data source, items, fixed layout, and selection. Future work should move toward visible disclosure UI, headers, column resizing, sorting behavior, editing, reuse identifiers, layout engines, and native accessibility. |
...
| 9: Tab Order And Focus Deep Dive | Planned | 0% | key-loop audit, focus-ring visibility, native Tab interception | Do a dedicated pass after the main control surface is broader. Validate first-responder state, visible focus indicators, child-control Tab interception, display-only control skipping, and Mac-style key-view ordering with targeted tests and a cleaner demo harness. |
...
- [x] Add first `NSPanel` subclass slice with common panel flags.
- [x] Add first `NSPopover` slice hosted by a menu-less borderless panel.
- [x] Add first `NSToolbar` / `NSToolbarItem` model slice with classic `ToolbarWindow32` rendering docked through `NSWindow.toolbar`.
- [ ] Add toolbar display modes for icon-only, label-only, and icon-and-label.
- [ ] Add toolbar resize behavior with flexible space and an overflow/more popup.
- [ ] Add AppKit-style toolbar customization UI for allowed items, default item set, drag ordering, and reset-to-default.
...
- [x] Add first deprecated-but-useful `NSForm` composed row slice.
- [x] Add first deprecated-but-useful `NSMatrix` composed button-grid slice.
...
- [x] Add first `NSOutlineView` tree-flattening slice over the table backend.
...
- [x] Add `WinFoundation.IndexPath` for `NSCollectionView` section/item addressing.
...
- [ ] Do a dedicated tab-order and focus-visibility deep dive after the main control surface is broader.
...
`NSScrollView` now owns an `NSClipView` content view, and the document view is hosted inside that clip view. The first `NSClipView` slice tracks `boundsOrigin`, `documentVisibleRect`, and document-frame offsetting in Swift. Native scrolling is still shallow, but the public hierarchy now matches AppKit more closely and gives future table/text/image scrolling a better foundation.
