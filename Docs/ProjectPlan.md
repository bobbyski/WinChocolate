...
| 3.25 | Auto Layout | ⏳ Pending | *(Promoted out of 3.4 on 2026-07-04.)* `NSLayoutConstraint`, layout anchors (`widthAnchor`/`leadingAnchor`/…), `NSStackView`, and `translatesAutoresizingMaskIntoConstraints` backed by a real constraint solver. This is a subsystem, not a name gap: stubbing anchors that silently don't lay out would be worse than their absence, so it is tracked separately rather than faked. WinChocolate ships frame + autoresizing-mask layout today; many programmatic AppKit apps use Auto Layout, so this unblocks a class of ports. Needs a Cassowary-style solver or a constraint-to-frame pass. |
...
- [x] ⤷ Auto Layout (constraints/anchors/`NSStackView`) → **3.25** (a solver subsystem, deliberately not faked)
