...
## Phase 5 — Tables, Lists, Collections 🔄 54%
...
| 5.5 | Cell and row view hosting | 🔄 In Progress | ~55% — the **framework-drawn table** is well under way (`NSTableViewDrawn.swift`). Slice 1: opting a view-based table in (`winUsesViewBasedCells`) realizes a custom-drawn peer that draws its own header/grid/alternating-rows/selection and **hosts the delegate's cell views as real child subviews** per cell, with click hit-testing → row selection and header-click sorting. Slice 2: **vertical scrolling** — as an `NSScrollView` document view the drawn table grows to its full content height and the existing scroll infrastructure clips + scrolls it (hosted cell views ride along). Slice 3: **variable row heights** — the drawn geometry (cell rects, hit-testing, content height, grid lines, hosted-view layout) honors the delegate's `heightOfRow`, cached per rebuild. Contract-tested (`testViewBasedTableHostsCellViews`, `testDrawnTableScrollsAsScrollViewDocument`, `testDrawnTableHonorsVariableRowHeights`) and demoed (10 scrolling rows with live text-field + button cells; completed rows grow to 44px live). Remaining slices: an in-place edit overlay, pinned header, `NSTableRowView` hosting, and auto-detecting view-based mode from the delegate to match AppKit. This subsystem also unblocks 5.2/5.4's depth. |
