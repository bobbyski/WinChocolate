...
| `NSPopUpButton` | `COMBOBOX` dropdown-list | `ComboBox` | Item and selection APIs, per-item tags (`selectedTag`/`selectItem(withTag:)`/`indexOfItem(withTag:)`), a retained `pullsDown` flag, and `autoenablesItems` + a per-item enabled model. Visual item graying, per-item images, and pull-down display need owner-draw (8.3). | Done | Planned |
...
| `NSSlider` | `msctls_trackbar32` | `Slider` | Native trackbar peer with value/range/action, tick marks (`numberOfTickMarks`/`allowsTickMarkValuesOnly`/`closestTickMarkValue`), vertical orientation, and a stored `altIncrementValue`. Tick-mark position is appearance-phase work. | Done | Planned |
...
| `NSLevelIndicator` | Progress bar/custom owner-drawn meter | Rating/value indicator pattern | Value/range state over a progress-style peer, `warningValue`/`criticalValue` bar recoloring (amber/red via `PBM_SETBARCOLOR`), and `isEditable` click/drag-to-set (a subclassed peer maps click x → value). Discrete/rating/relevancy visuals are appearance-phase work. | Done | Planned |
