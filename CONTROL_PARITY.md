...
| `NSFontPanel` | `ChooseFontW` dialog | Custom font picker | Shared panel + `NSFontManager` run the classic font chooser; a true floating panel is future work. | Partial | Planned |
| `NSColorPanel` | `ChooseColorW` dialog | `ColorPicker` dialog | Shared panel with color-well attachment runs the classic color chooser; a true floating panel is future work. | Partial | Planned |
