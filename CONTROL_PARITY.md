...
| `NSWindow` | Top-level `HWND` | `Window` / `AppWindow` | Key/main/title/frame behavior plus content size limits (`contentMinSize`/`contentMaxSize`/`minSize`/`maxSize` via `WM_GETMINMAXINFO`) and z-order levels. `fullSizeContentView`, standard-button customization, and state restoration are future work. | Partial | Planned |
...
| `NSButton` push button | `BUTTON` with push style | `Button` | Momentary push behavior plus `keyEquivalent` dispatch (Return/Escape fire default/cancel buttons through the key window's view chain). `bezelStyle`, images, and `alternateTitle` are future work. | Done | Planned |
...
| `NSTextField` editable | Single-line `EDIT` | `TextBox` | Editing, change notifications, `alignment` (`ES_CENTER`/`ES_RIGHT`), and `placeholderString` (`EM_SETCUEBANNER`; cue renders once comctl32 v6 visual styles are enabled). Formatter and delegate callbacks are future work. | Done | Planned |
...
| `NSColorWell` | Custom button/color swatch plus color dialog | `ColorPicker` | Clickable swatch presents the shared floating color panel; panel picks flow back into the active well live. | Partial | Planned |
...
| `NSSegmentedControl` | Composed `BUTTON` peers/custom owner draw | `Segmented` style via `RadioButtons`/custom | Composed segment state/action plus arrow-key selection (skips disabled segments); unified drawing, per-segment images/menus are future work. | Partial | Planned |
...
| `NSPopover` | Popup window/custom transient `HWND` | `TeachingTip` / `Flyout` | Transient popover over a menu-less borderless `NSPanel` with a solid background, `NSPopoverDelegate`, and outside-click dismiss for `.transient`/`.semitransient` (a `WH_MOUSE` thread hook); arrow/beak drawing and show/close animation are future work. | Partial | Planned |
| `NSAlert` | `MessageBoxW` + composed modal panel | `ContentDialog` | Custom button titles, suppression checkbox, style icon badge, and `accessoryView` run in a composed modal panel over `NSApplication.runModal`; plain alerts keep `MessageBoxW`. Sheets remain. | Partial | Planned |
| `NSPanel` | Tool/top-level `HWND` variants | Secondary window/dialog | Panel flags work against the backend: `isFloatingPanel` maps to a topmost tool window via `NSWindow.level`, `hidesOnDeactivate` hides on `WM_ACTIVATEAPP`, `.utilityWindow` styles compact chrome, and panels never become main. | Partial | Planned |
