...
| `NSTextView` | Multiline `EDIT` or RichEdit | `TextBox` multiline / `RichEditBox` | Multiline editing plus `selectedRange`, `insertText(_:replacementRange:)`, `scrollRangeToVisible`, delegate `textDidChange`, read-only sync, and fonts; rich text, find/replace, and undo remain. | Partial | Planned |
