...
| `NSTextField` editable | Single-line `EDIT` | `TextBox` | Editing, change notifications, `alignment` (`ES_CENTER`/`ES_RIGHT`), `placeholderString` (`EM_SETCUEBANNER`; cue renders once comctl32 v6 visual styles are enabled), and `NSTextFieldDelegate` begin/change/end editing over focus events. Formatter is future work. | Done | Planned |
...
| `NSDatePicker` | `SysDateTimePick32` / `SysMonthCal32` | `DatePicker`, `TimePicker`, `CalendarDatePicker` | Date value/min/max/action plus `datePickerElements` filtering (date, time `HH:mm:ss`, or date+time via `DTM_SETFORMATW`); clock-and-calendar style and a delegate are future work. | Partial | Planned |
